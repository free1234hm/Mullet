...
public class DataSetCore {
	...
	public void normalization_zcore() {
	...
	public void normalization_maxmin() {
		controlnorm = new double[numrows][numcols];
		for(int i=0;i<numrows;i++){
			double[] controlvalue = controldata[i];
			...
			for(int j=0;j<controlvalue.length;j++) {
				if(max < controlvalue[j]) max = controlvalue[j];
				if(min > controlvalue[j]) min = controlvalue[j];
			}
			for(int j=0;j<controlvalue.length;j++){
				controlnorm[i][j] = (controlvalue[j]-min)/(max-min);
			}
		}
	}
