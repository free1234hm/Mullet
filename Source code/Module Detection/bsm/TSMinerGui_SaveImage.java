...
public class TSMinerGui_SaveImage extends JPanel
{
	 ...
	 * Class constructor
	 */
	public TSMinerGui_SaveImage(final JFrame theFrame, HeatMap controlpanel, HeatMap casepanel)
	{
				...
				else if (state.equals(JFileChooser.APPROVE_SELECTION))
				{
					...
					try
					{
						...
						if(szext==null || szext.length()==0){
							File newFile = new File(file.getAbsolutePath() + "_positive.png");
							Dimension imageSize = controlpanel.getSize();
							...
							for(int i=0;i<sznames.length;i++){
								if(szext.equalsIgnoreCase(sznames[i])){
									File newFile = new File(file.getAbsolutePath()+"_positive"+szext);
									Dimension imageSize = controlpanel.getSize();
								    BufferedImage image = new BufferedImage(imageSize.width,imageSize.height, BufferedImage.TYPE_INT_RGB);
								    Graphics2D graphics = image.createGraphics();
								    controlpanel.paint(graphics);
						...
						/*************************************************************************/
						if(szext==null || szext.length()==0){
							File newFile = new File(file.getAbsolutePath() + "_negative.png");
							Dimension imageSize = casepanel.getSize();
						    BufferedImage image = new BufferedImage(imageSize.width,imageSize.height, BufferedImage.TYPE_INT_RGB);
						    Graphics2D graphics = image.createGraphics();
						    casepanel.paint(graphics);
						    graphics.dispose();
							ImageIO.write(image, "png", newFile);
						}else{
							for(int i=0;i<sznames.length;i++){
								if(szext.equalsIgnoreCase(sznames[i])){
									File newFile = new File(file.getAbsolutePath()+"_negative"+szext);
									Dimension imageSize = casepanel.getSize();
								    BufferedImage image = new BufferedImage(imageSize.width,imageSize.height, BufferedImage.TYPE_INT_RGB);
								    Graphics2D graphics = image.createGraphics();
								    casepanel.paint(graphics);
